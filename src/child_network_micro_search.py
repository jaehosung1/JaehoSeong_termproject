...
from tensorflow.keras.layers import Add, Concatenate, Reshape, multiply, add, Multiply
...
from tensorflow.keras.layers import Input, Conv2D, GlobalAveragePooling2D, Dropout, ReLU, concatenate
...
#########################################################################################
    def swish_dwconv2d_bn(self,
        ...
        x = DepthwiseConv2D(
            ...
            name="{0}_dwconv2d_{1}_".format(name_prefix, rep))(inputs)
        x = BatchNormalization(name="{0}_bn_{1}_".format(name_prefix, rep))(x)
        x = Activation(
            "swish", name="{0}_swish_{1}_".format(name_prefix, rep))(x)
        return x

    def dw_conv_3x3(self,  # 0
                    inputs,
                    name_prefix,
                    current_filters,
                    strides=(1, 1),
        ...
        tchannel = int(2.0 * current_filters)
        cchannel = int(1.0 * current_filters)
        r = strides == (1, 1) and input_shape[3] == current_filters
        x = Conv2D(tchannel, (1, 1), strides=(1, 1), padding='same',
                   name="{0}_expasion_".format(name_prefix))(inputs)
        x = BatchNormalization(name="{0}_bn_0_".format(name_prefix))(x)
        x = Activation(
            "swish", name="{0}_swish_0_".format(name_prefix))(x)
        x = self.swish_dwconv2d_bn(
            x,
            name_prefix,
            rep=1,
            kernel_size=(3, 3),
            strides=strides)
        x = Conv2D(cchannel, (1, 1), strides=(1, 1), padding='same',
                   name="{0}_contraction_".format(name_prefix))(x)
        x = BatchNormalization(name="{0}_bn_2_".format(name_prefix))(x)
        x = Dropout(0.1, name="{0}_drop_".format(name_prefix))(x)
        if r:
            x = Add(name="{0}_add_".format(name_prefix))([x, inputs])
            x = tfa.layers.StochasticDepth(name="{0}_stochastic_1_".format(name_prefix),
                                           survival_probability=1 - drop_connect)([inputs, x])
        return x

    def dw_conv_5x5(self,  # 1
                    inputs,
                    name_prefix,
                    current_filters,
                    strides=(1, 1),
                    drop_connect=.1):
        input_shape = K.int_shape(inputs)
        tchannel = int(1.5 * current_filters)
        cchannel = int(1.0 * current_filters)
        r = strides == (1, 1) and input_shape[3] == current_filters
        x = Conv2D(tchannel, (1, 1), strides=(1, 1), padding='same',
                   name="{0}_expasion_".format(name_prefix))(inputs)
        x = BatchNormalization(name="{0}_bn_0_".format(name_prefix))(x)
        x = Activation(
            "swish", name="{0}_swish_".format(name_prefix))(x)
        x = self.swish_dwconv2d_bn(
            x,
            name_prefix,
            rep=1,
    ...
    def max_pooling_3x3(self, inputs, name_prefix, strides=(1, 1)):  # 2
        x = MaxPooling2D(
            pool_size=(3, 3),
            strides=strides,
            padding="same",
    ...
    def average_pooling_3x3(self, inputs, name_prefix, strides=(1, 1)):  # 3
        x = AveragePooling2D(
            pool_size=(3, 3),
            strides=strides,
            padding="same",
    ...
    def identity(self, inputs, name_prefix):  # 4
        x = Activation(
            "linear", name="{0}_linear_".format(name_prefix))(inputs)
        return x

    def dilated_conv_3x3(self, inputs, name_prefix, current_filters, strides=(1, 1), dilation_rate=2,
                         drop_connect=.1):  # 5
        input_shape = K.int_shape(inputs)
        print("check dilated_conv dimension")
        print("dilated_conv_3x3's input:", inputs)
        r = strides == (1, 1) and input_shape[3] == current_filters
        if strides == (2, 2):
            x = self.swish_dwconv2d_bn(
                inputs,
            ...
            x = Conv2D(
                ...
                name="{0}_dilconv2d_".format(name_prefix))(inputs)
            x = BatchNormalization(name="{0}_bn_".format(name_prefix))(x)
            x = Activation(
                "swish", name="{0}_swish_".format(name_prefix))(x)
            print("dilated_conv_3x3's output:", x)
        if r:
            x = Add(name="{0}_add_".format(name_prefix))([x, inputs])
            x = tfa.layers.StochasticDepth(name="{0}_stochastic_1_".format(name_prefix),
                                           survival_probability=1 - drop_connect)([inputs, x])
        return x
    #
    # def dilated_conv_5x5(self, inputs, name_prefix, current_filters, strides=(1, 1), dilation_rate=2,
    #                      drop_connect=.2):  # 6
    #     input_shape = K.int_shape(inputs)
    #     r = strides == (1, 1) and input_shape[3] == current_filters
    #
    ...
    #                                        survival_probability=1 - drop_connect)([inputs, x])
    #     return x
    def activation(self, x, name_prefix, rep, func='swish'):
        '''
        Activation layer.
        '''
        return Activation(func, name="{0}_swish_{1}_".format(name_prefix, rep))(x)

    def se_module(self, inputs,name_prefix, rep=0, se_ratio = 4):
        '''
        ...
        print("inputs:")
        print(inputs)
        out_dim = inputs.get_shape().as_list()[3]
        print("out_dim:")
        print(out_dim)
        squeeze = GlobalAveragePooling2D(name="{0}_gap_{1}_".format(name_prefix, rep))(inputs)
        excitation = Dense(name="{0}_fc_{1}_".format(name_prefix, rep), units=out_dim // se_ratio)(squeeze)
        excitation = self.activation(excitation, name_prefix=name_prefix, rep=rep)
        rep +=1
        excitation = Dense(name="{0}_fc_{1}_".format(name_prefix, rep), units=out_dim)(excitation)
        excitation = self.activation(excitation, name_prefix=name_prefix, rep=rep, func='sigmoid')
        excitation = Reshape((1, 1, out_dim),name="{0}_reshape_".format(name_prefix))(excitation)
        #
        print("excitation:")
        print(excitation)
        scale = multiply([inputs, excitation], name="{0}_mul_".format(name_prefix))
        print("scale:")
        print(scale)
        return scale
...
class NetworkOperationController(object):
    ...
    def generate_node_operation(self,
        ...
        if oper_id == 0:
            return self.NO.dw_conv_3x3(
                inputs=inputs,
                name_prefix=name_prefix,
                current_filters=self.current_filters(),
                strides=strides)
        elif oper_id == 1:
            return self.NO.dw_conv_5x5(
                inputs=inputs,
                name_prefix=name_prefix,
                current_filters=self.current_filters(),
                strides=strides)
        ...
        #     r
            ...
            x = self.NO.se_module(inputs=inputs, name_prefix=name_prefix)
            if x.get_shape().as_list()[-1] != self.current_filters():
                x = self.NO.adjust_output_depth(
                    x,
                    name_prefix=name_prefix,
    ...
    def add_layers(self, x_0, x_1, node_num, reduction=False,op_id=[]):
        """
    ...
    """
        ...
        print("x_1")
        print(x_1)
        x = Add(name="{0}_add_".format(name_prefix))([x_0, x_1])
        print("add")
        print("x")
        print(x)
        return x
    ...
    """
        ...
        # x = Dropout(0.1, name="{0}_drop_{1}".format(name_prefix,rep))(x)
        rep +=1
        return x
    ...
    """
        ...
        for n, lrop in node_operation.items():
            ...
            for lr, op in lrop.items():
                ...
                # if op["oper_id"] == 6:
                #     node_output[n] = self.NOC.mul_layers(
                #     lr_outputs["L"],
                #     lr_outputs["R"],
                #     node_num=n,
                #     reduction=reduction
            print("#node", n)
            node_output[n] = self.NOC.add_layers( # mul도함
                lr_outputs["L"],
                lr_outputs["R"],
                node_num=n,
                reduction=reduction,
...
class ChildNetworkController(object):
    ...
    """
        for l in range(len(self.model.layers)):
            ...
            # print("model_name[1]$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$",model_name[1])
            _model_dict[l] = {
    ...
    def train_child_network_best(self,
                            train_ds,
                            x_train,
                            y_train,
                            validation_data=None,
         ...
         plot_process(hist, 'Training process')
